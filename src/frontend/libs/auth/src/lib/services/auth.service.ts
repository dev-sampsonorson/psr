...
export class AuthService {
    ...
    public getRefreshToken(): Observable<string | null> {
        ...
        return this.userStorage.getUser().pipe(
            take(1),
            map(user => user && user.refreshToken)
        );
    }
    ...
    public logout() {
        // console.log('this.userStorage.setUser(null);');
        // this.userStorage.setUser(null);
        // this.userStorage.removeUser();
        // logout from server
        // pass refresh token if you can
        // console.log('logging out...');
        /* return this.http.post<boolean>(this.appConfig.getUrl(AuthEndpoints.RevokeToken), {}, { withCredentials: true }).pipe(
            // shareReplay(),
            tap(() => {
        ...
        return this.userStorage.getUser().pipe(
            take(1),
            map(user => user && user.refreshToken),
            // TODO: Should i logout user if user doesn't exist in local storage
            // if user in local storage is null,
            // should i log the user out on client
            // knowing cookie might still have the refresh token
            switchMap(refreshToken => this.http.post<boolean>(this.appConfig.getUrl(AuthEndpoints.RevokeToken), { refreshToken }, { withCredentials: true }).pipe(
                // shareReplay(),
                tap(() => {
                    this.userStorage.removeUser();
                    this.router.navigate([this.appConfig.navigationPaths.Login]);
